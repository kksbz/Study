...
namespace WhatIsDelegate
{
    ...
    {
        //void 리턴타입과, 매개변수를 받지 않는 함수를 저장할 수 있는 데이터 타입을 선언한 것.
        ...
        public void DelegateDesc()
        {
             ...
             * 영어 단어 delegate는 위임하다 또는 대신하다의 의미가 있다.
             * 
             * 대리자(위임/델리게이트)
             ...
             * 
             * - 자동차 개체를 예로 들면, 대리운전처럼 대리자가 집까지 좌회전(), 우회전(),
             * 직진(), 주차() 등 동작을 대신해서 할 수 있게 하는 개념과 비슷하다.
             * - 메서드의 매개변수로 대리자를 넘길 수 있다. 대리자를 사용하여 함수의
             *   매개변수로 함수 자체를 전달할 수 있다.
             ...
             * 
             * action func과 동일한 기능 (action func이 올드함 delegate로 다 구현됨)
             */

            SayDelegate sayDelegate = Hi; //함수를 담음
            sayDelegate.Invoke(); //Invoke()로 함수 호출함
