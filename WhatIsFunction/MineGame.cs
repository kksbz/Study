...
namespace WhatIsFunction
{
    ...
    internal class MineGame
    {
        static void Main(string[] args)
        {
             ...
             * -2: 지뢰 있음
             * -1: 초기값
             * n: 주변 9타일 이내에 지뢰 수 (0일 경우 ■표시, 양수일 경우 정수 표기)
             * 
             * mineCntMap 상태
             * -1: 지뢰 있음
             * n: 주변 9타일 이내에 지뢰 수
            ...
            while(isGameOver == false)
            {
                ...
                while(isLocationValid == false)
                {
                    ...
                    //좌표를 제대로 입력한 경우만 이 아래로 코드가 진행됨. 왜냐면 유효하지 않은 경우 위에서 continue를 만나기 때문에
                    ...
                    //좌표를 제대로 입력한 경우만 이 아래로 코드가 진행됨. 왜냐면 유효하지 않은 경우 위에서 continue를 만나기 떄문에
                } // loop
                playerTurnCnt++;
                //}플레이어 좌표 입력

                //현재 턴이라면 해당 좌표에 지뢰가 있어도 지워준다
                    ...
                    for(int y=0;y< BOARD_SIZE_Y;y++)
                    {
                        for(int x=0;x< BOARD_SIZE_X;x++)
                        {
                            ...
                            { 
                                ...
                                {
                                    //유효하지 않은 좌표는 패스한다
                                    isSearchTileValid = (0 <= searchX && searchX < BOARD_SIZE_X) &&
                ...
                } // else: 지뢰 아닌, 빈 타일을 선택한 경우, }선택한 타일 인근 9칸의 숫자를 오픈한다
                //}선택한 좌표에서 지뢰를 검사한다
